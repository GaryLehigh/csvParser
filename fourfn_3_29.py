...
class BinaryTree(object):
    ...
    def traverseTree(self, root):    
        if ( root.result == 0 or root.result == False ) and root.type == comOp:
            ...
            elif ( root.l_child.result == 0 or root.l_child.result == False ) and root.l_child.type == not_comOp:
                print(root.data)
                root.l_child.isKeyNode = isKeyNode
                root.l_child.flag = self.nodePairFlag
                root.l_child.data = root.l_child.data + '(' + str( self.nodePairFlag ) +')'
                
                
            if root.r_child.result == 0 :
                self.traverseTree(root.r_child)
            elif ( root.r_child.result == 0 or root.r_child.result == False ) and root.r_child.type == not_comOp:
                print(root.data)
                root.r_child.isKeyNode = isKeyNode
                root.r_child.flag = self.nodePairFlag
                root.r_child.data = root.r_child.data + '(' + str( self.nodePairFlag ) +')'    
